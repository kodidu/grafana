...
func (f *accessControlDashboardPermissionFilter) addPermissionCheck(query *strings.Builder, actions []interface{}, rolesFilter string, rolesFilterArgs []interface{}, args []interface{}) {
	// TODO extract permissio selection logic
	query.WriteString("(dashboard.uid IN (SELECT substr(scope, 16) FROM permission WHERE scope LIKE 'dashboards:uid:%'")
	query.WriteString(rolesFilter)
	args = append(args, rolesFilterArgs...)

	if len(actions) == 1 {
		query.WriteString(" AND action = ?")
		args = append(args, actions[0])
	} else {
		query.WriteString(" AND action IN (?" + strings.Repeat(", ?", len(actions)-1) + ") GROUP BY role_id, scope HAVING COUNT(action) = ?")
		args = append(args, actions...)
		args = append(args, len(actions))
	}
